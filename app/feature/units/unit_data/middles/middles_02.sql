...
WITH major_inserts AS (
    INSERT INTO majors (title, sort_order, is_published, textbook_id)
        VALUES
            ('수와 연산', 1, true, 22), -- textbook_id는 중학교 2학년 교재 ID로 가정
            ('변화와 관계', 2, true, 22),
            ('도형과 측정', 3, true, 22)
...
FROM (VALUES
          ...
          -- 상대도수와 확률 - 소단원들
          ('상대도수', 1, '상대도수와 확률', '성취기준: [9수04-03] - 상대도수의 개념과 분포 표현'),
          ('경우의 수와 확률', 2, '상대도수와 확률', '성취기준: [9수04-05], [9수04-06] - 경우의 수와 확률의 개념')
...
WITH curriculum_unit_mapping AS (
    ...
    FROM (
             VALUES
                 ...
                 -- 상대도수
                 ((SELECT unit_id FROM units WHERE title = '상대도수'), ARRAY['9수04-03']),

                 -- 경우의 수와 확률
                 ((SELECT unit_id FROM units WHERE title = '경우의 수와 확률'), ARRAY['9수04-05', '9수04-06'])
