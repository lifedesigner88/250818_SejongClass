...
import type { UnitInfoType } from "~/feature/textbooks/layout/textbook-layout";
...
export const action = async ({ request }: Route.ActionArgs) => {
    ...
    if (isAdmin) {
        ...
        await updateMajor(unitInfo.major.id, unitInfo.major.title, unitInfo.major.sort_order);
        await updateMiddle(unitInfo.middle.id, unitInfo.middle.title, unitInfo.middle.sort_order);
        await updateUnit(
            unitInfo.unit.id, unitInfo.unit.title, unitInfo.unit.sort_order,
            unitInfo.is_free, unitInfo.is_published
        );
    }
    return { "ok": true }
}